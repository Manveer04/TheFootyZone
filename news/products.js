...
const containsBoot = /boot/i.test(productId);
const containsJersey = /jersey/i.test(productId);
const containsBalls = /ball/i.test(productId);
let product;
...
const productIdMapping = {
  ...
  const productIndex = productIdMapping[productId];
  product = productsData["Boots"][productIndex];
  product.category = "Boots";
} else if(containsJersey){
    const productIdMapping = {
        "jersey1": 0,
        "jersey2": 1,
        "jersey3": 2,
      ...
      const productIndex = productIdMapping[productId];
      product = productsData["Jerseys"][productIndex];
      product.category = "Jerseys";

} else if(containsBalls){
    const productIdMapping = {
        "ball1": 0,
        "ball2": 1,
        "ball3": 2,
      ...
      const productIndex = productIdMapping[productId];
      product = productsData["Balls"][productIndex];
...
if (product != 0) {
    const productDetailsHtml = document.createElement('div');
    productDetailsHtml.innerHTML = `
    <div  id="specific-product">
    <section id="productdel" class="padding_default">
    <div id="single-product-img">
        <img src="../${product.imageSrc}" width="100%" id="MainImage" alt="Sewy1">
        <div class="small-img-group">
            <div class="small-img-col">
                <img src="../${product.imageSrc}" width="100%" class="small-image" alt="Sewy1">
            </div>
            <div class="small-img-col">
                <img src="../${product.imageSrc1}" width="100%" class="small-image" alt="Sewy2">
            </div>
            <div class="small-img-col">
                <img src="../${product.imageSrc2}" width="100%" class="small-image" alt="Sewy3">
            </div>
            <div class="small-img-col">
                <img src="../${product.imageSrc3}" width="100%" class="small-image" alt="Sewy4">
            </div>
        </div>
    </div>

    <div class="single-productdel">
    <h6><a href="shop.html">Home</a>/${product.category}</h6> <!-- Display the category here -->
    <h3>${product.brand}</h3>
    <h4 class="productname">${product.name}</h4>
    <h2 class="productprice">${product.price}</h2>
    <button class="add-to-cart-btn" type="normalbutton">Add to Cart</button>
    ...
    productDetailsContainer.innerHTML = ''; // Clear any previous content
    productDetailsContainer.appendChild(productDetailsHtml);
} else {
    // Handle the case when the product is not found
    document.getElementById('productDetails').innerHTML = '<p>Product not found.</p>';
}
...
function createProductBoxShop(product) {
    ...
    <a class="productpg" href="productdetails.html?id=${product.productId}"><img src="../${product.imageSrc}" alt="${product.name}"></a>
        <div class="description">
            ...
            <h4>${product.price}</h4>
            <div class="shcart">
                <a href="#"><i class="fal fa-shopping-cart carticon"></i></a>
            </div>
        </div>
    `;
    return productBox;
...
function displayOtherProducts() {
    var featuredProductsContainer = document.getElementById("other-products-container");
...
function addToCart(event) {
    const productBox = event.target.closest("#productdel");
    ...
    const productImage = productBox.querySelector(".small-image").src;
